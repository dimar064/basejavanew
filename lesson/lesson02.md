...
> Правка к видео: в `storage.ArrayStorage.delete()` вместо `storage[i] = null` нужно записать `storage[size - 1] = null`
...
- Реализуйте и протестируйте `storage.ArrayStorage.update(model.Resume resume)`
- Сделайте проверки:
  - в `update/delete/get` - резюме есть в `storage`?
  - в `save`- резюме нет в `storage`?
  - сделайте в `save` проверку на переполнение
  - выведите соответствующие предупреждения для всех, указанных выше проверок - `System.out.println("model.Resume ...")`
- Избавьтесь от дублирования в коде `storage.ArrayStorage`
